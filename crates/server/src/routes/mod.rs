...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            ...
            post(auth::create_desktop_pairing_request),
        )
        .route(
            "/desktop/pairing-requests/{code}",
            get(auth::get_desktop_pairing_request_status),
        )
        .route(
            "/desktop/pairing-requests/{code}/approve",
            post(auth::approve_desktop_pairing_request),
        )
