...
const activeProjectPath = computed(() => selectedProject.value?.path ?? ws.selectedProjectPath.value);
const activeSessionId = computed(() => activeProjectPath.value ? ws.projectShellSessionId(activeProjectPath.value) : "");
...
onMounted(() => {
  ...
  terminal.onData((data) => {
    ...
    ws.sendProjectShellInput(activeProjectPath.value, data).catch(() => undefined);
  });
...
function startProjectShell(forceRestart = false) {
  const path = activeProjectPath.value;
  if (!path) {
    void ws.chooseProject();
    return;
  }
  void ws.startShellForProject(path, forceRestart);
}
...
async function fitTerminal() {
  ...
  try {
    ...
    if (dimensions) {
      await ws.resizeProjectShell(activeProjectPath.value, dimensions.cols, dimensions.rows);
    }
  } catch {
  ...
  <div class="terminal-frame" :class="{ 'no-session': terminalState === 'no-session' }">
    ...
    <div v-if="terminalState !== 'no-session'" class="terminal-bottombar">
      <div class="terminal-tabs">
        <span class="terminal-tab active">{{ selectedProject?.name ?? "项目 shell" }}</span>
      </div>
      <button
        v-if="terminalState === 'ready'"
        class="terminal-restart-button"
        type="button"
        @click="startProjectShell(true)"
      >
        重启 shell
      </button>
    </div>
    <div v-if="terminalState === 'no-session'" class="terminal-session-placeholder">
      <div class="terminal-session-placeholder-inner">
        ...
        <div class="terminal-placeholder-actions">
          <button v-if="selectedProject" class="button primary narrow" type="button" @click="startProjectShell()">
            打开 shell
          </button>
    ...
    <div v-else-if="terminalState !== 'ready'" class="terminal-empty-state" :class="terminalState">
      ...
      <button v-if="terminalState === 'offline'" type="button" @click="startProjectShell()">
        启动 shell
      </button>
    </div>
  </div>
