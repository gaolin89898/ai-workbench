...
import { ref } from "vue";
import type { WorkspaceProject } from "../services/tauri";

defineProps<{
  projects: WorkspaceProject[];
  projectResult: string;
  projectResultError: boolean;
}>();
...
<template>
  <section class="view active" data-view-panel="projects">
    <header class="topbar">
      <div>
        <h1>项目</h1>
        <p>选择本机项目，然后创建新 AI 会话或接管已有会话。</p>
      </div>
      <button class="button primary narrow" type="button" @click="emit('chooseProject')">选择本地项目</button>
    </header>
    <section class="workspace-grid two-columns">
      <article class="panel">
        ...
        <div class="button-row">
          <button class="button primary narrow" type="button" @click="emit('chooseProject')">打开文件夹选择器</button>
          <button class="button secondary narrow" type="button" @click="emit('addProject', projectPath)">手动添加路径</button>
        </div>
        <div class="result-box" :class="{ error: projectResultError }">{{ projectResult }}</div>
      </article>
      <article class="panel">
        <h2>项目列表</h2>
        <p class="hint-text">项目是主入口。AI 会话会挂在项目下面，底层 tmux/screen 只作为承载层。</p>
