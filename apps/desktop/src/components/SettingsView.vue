...
<template>
  <section class="view active settings-page" data-view-panel="settings">
    <aside class="settings-nav">
      <button class="settings-back-button" type="button" @click="emit('switchView', 'aiSessions')">← 返回首页</button>
    ...
    <div class="settings-content">
      <div class="settings-scroll">
        <header class="settings-header">
          ...
          <button class="button primary narrow" type="button" @click="emit('saveSettings')">保存设置</button>
        </header>
      ...
      <section v-else-if="settingsPanel === 'pairing'" class="settings-section">
        ...
        <div class="settings-card settings-form-card">
          ...
          <button class="button primary" type="button" @click="emit('pairDesktop', localServer, pairingCode)">配对这台桌面</button>
          <div class="settings-pair-result" :class="{ error: pairResultError }">{{ pairResult }}</div>
        </div>
      </section>

      <section v-else class="settings-section">
        <h2 class="settings-section-title">调试入口</h2>
        <p class="settings-section-description">这些入口主要用于排查 Provider 检测和底层 tmux / screen 承载状态。</p>
        <div class="settings-card">
          <label class="settings-row">
            <span class="settings-row-copy">
              <strong>底层终端</strong>
              <small>保留 tmux / screen 调试信息</small>
            </span>
        ...
        <div class="settings-result">{{ settingsResult }}</div>
      </div>
    </div>
  </section>
</template>
