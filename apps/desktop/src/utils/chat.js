...
export function encodeAssistantMessageForStorage(message) {
    return `${STRUCTURED_MESSAGE_PREFIX}${JSON.stringify({
        text: message.text,
        segments: message.segments,
    })}`;
}
export function decodeAssistantMessageFromStorage(value) {
    if (!value.startsWith(STRUCTURED_MESSAGE_PREFIX))
        return { text: value };
    try {
...
export function cleanAssistantOutput(output, prompt) {
    return assistantDisplayLines(output, prompt)
        .join("\n")
        .trim();
}
export function assistantOutputToSegments(output, prompt) {
    ...
    while (index < lines.length) {
        ...
        while (index < lines.length) {
            ...
            if (parseToolStart(nextNormalized))
                break;
            if (isLikelyNarrativeLine(nextNormalized))
                break;
...
export function formatChatMessageText(text) {
...
function extractTextFromJsonValue(value) {
    ...
    if (record.delta && typeof record.delta === "object")
        return extractTextFromJsonValue(record.delta);
    return "";
}
function normalizeChatText(value) {
    return value.replace(/^[>›$❯❮┃|│•\s]+/, "").trim();
}
function stripAssistantDisplayPrefix(value) {
    return value.replace(/^\s*•\s?/, "");
}
function assistantDisplayLines(output, prompt) {
...
function cleanupAssistantLine(value) {
    return value
...
function shouldKeepAssistantLine(normalizedLine, normalizedPrompt) {
...
function isPromptEchoLine(normalizedLine, normalizedPrompt) {
...
function isLikelyNarrativeLine(value) {
    if (!value)
        return false;
    if (value.startsWith("我") || value.startsWith("这个") || value.startsWith("目录") || value.startsWith("项目"))
        return true;
    if (/[\u4e00-\u9fff]/.test(value) && !/^[A-Z?]{1,2}\s/.test(value))
        return true;
    return false;
}
function isTerminalStatusLine(value) {
    return (isBoxDrawingLine(value) ||
        /^0;/.test(value) ||
...
function isBoxDrawingLine(value) {
    return /^[╭╮╰╯─│\s_]+$/.test(value);
}
function isToolTraceLine(value) {
