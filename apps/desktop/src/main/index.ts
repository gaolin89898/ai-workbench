...
const APP_USER_MODEL_ID = "com.gaolin89898.aiworkbench";
...
function appIconPath(): string | undefined {
  ...
  const candidates = app.isPackaged
    ? [join(process.resourcesPath, "build", iconFile), join(process.resourcesPath, "build", "icon.png")]
    : [join(__dirname, "../../build", iconFile), join(__dirname, "../../build", "icon.png")];
...
function createAppIcon() {
  ...
  return iconPath ? nativeImage.createFromPath(iconPath) : nativeImage.createFromDataURL(TRAY_ICON_DATA_URL);
}
