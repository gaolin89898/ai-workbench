...
function providerDisplayName(providerId) {
    ...
    return providers.value.find((provider) => provider.id === providerId)?.name
        ?? { codex: "Codex", claude: "Claude Code", opencode: "OpenCode", deepseek: "DeepSeek" }[providerId]
        ?? "AI";
}
...
async function loadAiSessionHistory(sessionId) {
    try {
...
function nextPendingTextStepId(pending) {
...
function upsertPendingSegment(sessionId, segment) {
    ...
    if (segment.type === "status") {
        if (shouldHideBackendStatus(segment.label))
            return;
        if (pending.lastStatusText === segment.label)
            return;
        ...
        if (segment.label.includes("会话已连接")) {
            ...
            pending.steps.set("runtime-status", { ...segment, stepId: "runtime-status" });
        }
        syncPendingAssistantSegments(sessionId, pending.message.pending === false);
        return;
    }
    ...
    if (isNewStep)
        pending.currentTextStepId = null;
...
function syncPendingAssistantSegments(sessionId, done = false) {
    ...
    if (!pending)
        return;
    const segments = [...pending.steps.values()].filter((segment) => (!done || segment.type !== "status" || isPersistentStatusSegment(segment)));
    patchPendingAssistant(sessionId, {
        pending: !done,
        role: "assistant",
...
function isPersistentStatusSegment(segment) {
    return segment.type === "status" && (segment.stepId === "final-summary");
}
...
function renameUntitledSession(sessionId, prompt) {
...
async function pairDesktop(server, code) {
