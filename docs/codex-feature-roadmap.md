...
| 21 | Subagents 任务树 | 部分接入：trace 解析 `subAgentActivity`（启动/交互/中断状态 + 代理路径，显示为"子代理：xxx"段）与 `collabAgentToolCall`（spawnAgent 等子代理工具调用，显示为"子代理工具：xxx"段）；完整父子代理树形 UI 与并行进度视图待后续 | 中 | P2 |
